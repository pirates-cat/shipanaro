...
class Command(BaseCommand):
        ...
        try:
            ...
            with transaction.atomic():
                for email in users:
                    ...
                    s, remaining = self.__load_subscription(u, remaining)
                    if len(remaining) > 0:
    ...
    def __load_membership(self, data):
        ...
        mp = m.payload
        if mp.date_left:
            mp.date_left = pytz.utc.localize(datetime.strptime(
                mp.date_left, '%Y-%m-%d'))
        mp.birthday = pytz.utc.localize(datetime.strptime(
            mp.birthday, '%Y-%m-%d'))
        return (mp, data)
    ...
    def __load_user(self, data):
        ...
        up.date_joined = pytz.utc.localize(datetime.strptime(
            up.date_joined, '%Y-%m-%d'))
        if up.last_login:
            up.last_login = pytz.utc.localize(datetime.strptime(
                up.last_login, '%Y-%m-%d'))
        return (up, data)

    def __load_subscription(self, user, data):
        s = None
        if 'newsletter' in data:
            s = Subscription()
            s.service = 'newsletter'
            if isinstance(user.email, list):
                s.endpoint = user.email[1]
            else:
                s.endpoint = user.email
            del data['newsletter']
        return (s, data)
